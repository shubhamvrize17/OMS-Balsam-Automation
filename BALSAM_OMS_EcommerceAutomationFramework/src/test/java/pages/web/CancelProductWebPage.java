...
public class CancelProductWebPage extends BasePage {
	...
	@FindBy(xpath = "//a[text() = 'Cancel Products']")
	...
	public void clickCancelProductsLink() {
		...
		jsClick(CancelProductsLink, WaitLogic.CLICKABLE, "CancelProducts");
	}
	...
	public void clickChoseanOption() {
		// click(By.xpath("(//div[@widgetid='idx_form_RadioButtonSet_1_RadioItem1']//input)[1]"),
		// WaitLogic.CLICKABLE, "ChoseanOption");
		WebDriverWait wait = new WebDriverWait(ldriver, Duration.ofSeconds(5));
		By by = By.xpath("(//div[@aria-label='Choose an option:']//input)[3]");
		wait.until(ExpectedConditions.elementToBeClickable(by));
		//ChoseanOption.click();
		jsClick(ChoseanOption, WaitLogic.CLICKABLE, "ChoseanOption");
	}

	public void selectCheckBtn() {
		WebDriverWait wait = new WebDriverWait(ldriver, Duration.ofSeconds(5));
		By by = By.xpath("(//div[@uid='OLST_listGrid']/descendant::span[@class= 'gridxIndirectSelectionCheckBox dijitReset dijitInline dijitCheckBox '])[2]");
		wait.until(ExpectedConditions.elementToBeClickable(by));
		
		click(CheckBtn, WaitLogic.CLICKABLE, "CheckBtn");
	}

	public void partialselectCheckBtn() {
		WebDriverWait wait = new WebDriverWait(ldriver, Duration.ofSeconds(5));
		By by = By.xpath("(//div[@uid='OLST_listGrid']/descendant::span[@class= 'gridxIndirectSelectionCheckBox dijitReset dijitInline dijitCheckBox '])[3]");
		wait.until(ExpectedConditions.elementToBeClickable(by));
		click(partialCheckBtn, WaitLogic.CLICKABLE, "partialCheckBtn");
	}
